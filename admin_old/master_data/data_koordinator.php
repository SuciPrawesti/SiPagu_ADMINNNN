<?php
/**
 * MASTER DATA KOORDINATOR - SiPagu
 * Lokasi: admin/master_data/data_koordinator.php
 * Fungsi: Read, Update, Delete data koordinator
 */
...
if (isset($_GET['delete_id'])) {
    ...
    if ($has_relations) {
        $_SESSION['message'] = 'Data koordinator tidak dapat dihapus karena masih digunakan di tabel lain.';
        $_SESSION['message_type'] = 'danger';
    } else {
        $delete_query = mysqli_query($koneksi, 
            "DELETE FROM t_user WHERE id_user = $id_to_delete AND role_user = 'koordinator'"
        );
        
        if ($delete_query) {
            $_SESSION['message'] = 'Data koordinator berhasil dihapus.';
            $_SESSION['message_type'] = 'success';
        } else {
            $_SESSION['message'] = 'Gagal menghapus data koordinator: ' . mysqli_error($koneksi);
            $_SESSION['message_type'] = 'danger';
        }
    }
    
    header("Location: data_koordinator.php");
    exit();
}
...
if (isset($_GET['edit_id'])) {
    ...
    $edit_result = mysqli_query($koneksi, 
        "SELECT * FROM t_user WHERE id_user = $edit_id AND role_user = 'koordinator'"
    );
    ...
    if (!$edit_result) {
        ...
        $_SESSION['message'] = 'Data koordinator tidak ditemukan.';
        $_SESSION['message_type'] = 'danger';
        header("Location: data_koordinator.php");
        exit();
    }
...
// ==================== PROSES UPDATE ====================
if (isset($_POST['update_koordinator'])) {
    ...
    if ($update_query) {
        $_SESSION['message'] = 'Data koordinator berhasil diperbarui.';
        $_SESSION['message_type'] = 'success';
        header("Location: data_koordinator.php");
        exit();
    } else {
        $message = 'Gagal memperbarui data koordinator: ' . mysqli_error($koneksi);
        $message_type = 'danger';
    }
}
...
// Query data koordinator
$query = mysqli_query($koneksi, 
    "SELECT * FROM t_user WHERE role_user = 'koordinator' ORDER BY nama_user ASC"
);
...
// Hitung total koordinator
$total_query = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM t_user WHERE role_user = 'koordinator'");
$total_data = mysqli_fetch_assoc($total_query);
$total_koordinator = $total_data['total'];
?>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <?php if ($edit_mode): ?>
                            <!-- Form Edit Koordinator -->
                            <div class="edit-form-container">
                                <h5>Edit Data Koordinator</h5>
                                <form method="POST" action="" class="edit-form">
                                    ...
                                    <div class="form-group text-right">
                                        <a href="data_koordinator.php" class="btn btn-secondary">Batal</a>
                                        <button type="submit" name="update_koordinator" class="btn btn-primary">
                            ...
                            <!-- Tabel Data Koordinator -->
                            <div class="table-responsive">
                                <table class="table table-striped table-hover" id="table-koordinator">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>NPP</th>
                                    ...
                                    <tbody>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <a href="?delete_id=<?= $row['id_user'] ?>" 
                                                       ...
                                                       onclick="return confirm('Yakin ingin menghapus data koordinator <?= htmlspecialchars($row['nama_user']) ?>?')">
                                        ...
                                        <tr>
                                            <td colspan="9" class="text-center">
                                                <?= $message ? $message : 'Tidak ada data koordinator' ?>
                                            </td>
                                        </tr>
                                        <?php endif; ?>
                                    </tbody>
                        ...
                        <div class="card-footer text-right">
                            <small class="text-muted">
                                Total: <?= $total_koordinator ?> Koordinator
                            </small>
                        </div>
                    </div>
                </div>
